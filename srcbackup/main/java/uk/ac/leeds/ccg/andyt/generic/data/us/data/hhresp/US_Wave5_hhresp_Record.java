...
public class US_Wave5_hhresp_Record extends US_hhresp_Record {
...
protected final void initHSIVW4(String s) {
if (!s.trim().isEmpty()) {
HSIVW4 = Byte.parseByte(s);
} else {
HSIVW4 = Byte.MIN_VALUE;
}
}
...
protected final void initXPBLDF(String s) {
if (!s.trim().isEmpty()) {
XPBLDF = Byte.parseByte(s);
} else {
XPBLDF = Byte.MIN_VALUE;
}
}
...
public US_Wave5_hhresp_Record(String line) {
s = line.split("\t");
initHID(s[0]);
initHIDP(s[1]);
initINTDATED(s[2]);
...
initNA75PL(s[118]);
initAGECHY_DV(s[119]);
initNCOUPLE_DV(s[120]);
initNONEPAR_DV(s[121]);
initNEMP_DV(s[122]);
initNWAGE_DV(s[123]);
initTENURE_DV(s[124]);
initXPHSN(s[125]);
