...
import uk.ac.leeds.ccg.data.us.core.US_Strings;
 ...
 * written to uk.ac.leeds.ccg.generic.data.us.data.hhold. Source code classes
 * written in order to load the US individual data is written to
 * uk.ac.leeds.ccg.generic.data.us.data.person.
 */
public class US_JavaCodeGenerator extends US_Object {
    ...
    public static void main(String[] args) {
        try {
            US_JavaCodeGenerator p = new US_JavaCodeGenerator(new US_Environment(
                    new Data_Environment(new Generic_Environment(
                            new Generic_Defaults()))));

            //p.files.setDataDirectory(Paths.get(System.getProperty("user.dir"), "data"));
//        String type;
//        type = "hhold";
            String name;
    ...
    protected Object[] getFieldTypes(String name, int n,
            HashMap<Short, String> WaveUKHLSNameLookup,
            HashMap<Short, String> WaveBHPSNameLookup) throws IOException {
        Object[] r;
        r = new Object[4];
        Path indir = files.getUSInputDir();
        HashMap<String, Integer>[] allFieldTypes = new HashMap[n];
        String[][] headers = new String[n][];
        HashMap<String, Byte>[] v0ms = new HashMap[n];
        HashMap<String, Byte>[] v1ms = new HashMap[n];
        Object[] t;
    ...
    public Object[] loadTest(short wave, String type, String name,
            HashMap<Short, String> waveNameLookup, Path indir) throws IOException {
        ...
        byte[] v0;
        byte[] v1;
        String swave = waveNameLookup.get(wave);
        Path f = US_Data.getInputFile(name, swave, indir);
        String m = "Test load " + type + " wave " + wave + " US " + "data from " + f;
    ...
    public void run(String name, int n, Object[] types) throws IOException {
        HashMap<String, Integer> fieldTypes;
        ...
        Path outdir = Paths.get(files.getDataDir().getParent().toString(),
                "src", "main", "java", "uk", "ac", "leeds", "ccg", "data", 
                "us", "data", name);
        Files.createDirectories(outdir);
        String packageName = "uk.ac.leeds.ccg.data.us.data." + name;
